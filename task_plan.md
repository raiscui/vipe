...
**已完成**: MoGe v2 默认生效,代码与记录均已提交并推送到 `raiscui/vipe`。

# -----------------------------------------------------------------------------
# 任务计划: 修复 Video-Depth-Anything 权重缓存损坏导致崩溃(vipe)
# -----------------------------------------------------------------------------

## 目标

当 `vipe infer -p lyra` 进入后处理阶段加载 Video-Depth-Anything 权重时:

- 如果本地缓存文件损坏(典型报错: `failed finding central directory`),程序能自动检测并重试下载.
- 避免用户手工清缓存后再跑一遍,减少重复 SLAM 计算时间和踩坑成本.
...
1. 当前使用 `torch.hub.load_state_dict_from_url` 下载 HF 权重,缓存损坏时会直接崩溃.
2. 在网络不稳定/代理环境下,需要更稳的下载与缓存策略,尽量支持断点续传或原子落盘.

## 状态

**目前在阶段5**: 提交并推送到 `raiscui/vipe`,并把修复过程追加到 WORKLOG/ERRORFIX。
